...
DELIMITER $$
USE `Catastro`$$
CREATE DEFINER = CURRENT_USER TRIGGER `Catastro`.`ViviendaUNIF_Persona_Vive_BEFORE_INSERT` BEFORE INSERT ON `ViviendaUNIF_Persona_Vive` FOR EACH ROW
BEGIN
	SET @vive = (SELECT COUNT(Portal) FROM Piso_Persona_Vive WHERE DNI=NEW.DNI);
	SET @vive1 = (SELECT COUNT(Numero) FROM ViviendaUNIF_Persona_Vive WHERE DNI=NEW.DNI);
...
USE `Catastro`$$
CREATE DEFINER = CURRENT_USER TRIGGER `Catastro`.`Piso_Persona_Vive_Ya_Vive_BEFORE_INSERT` BEFORE INSERT ON `Piso_Persona_Vive` FOR EACH ROW
BEGIN
	SET @vive = (SELECT COUNT(Portal) FROM Piso_Persona_Vive WHERE DNI=NEW.DNI);
	SET @vive1 = (SELECT COUNT(Numero) FROM ViviendaUNIF_Persona_Vive WHERE DNI=NEW.DNI);
	IF (@vive>0 OR @vive1>0) THEN
